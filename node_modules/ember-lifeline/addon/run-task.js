...
/**
   ...
   @public
   */
export function runTask(destroyable, taskOrName, timeout = 0) {
    assert(`Called \`runTask\` on destroyed object: ${destroyable}.`, !destroyable.isDestroyed);
...
/**
   ...
   @public
   */
export function scheduleTask(destroyable, queueName, taskOrName, ...args) {
    ...
    assert(`Called \`scheduleTask\` on destroyed object: ${destroyable}.`, !destroyable.isDestroyed);
...
/**
   ...
   @public
   */
export function throttleTask(destroyable, taskName, ...throttleArgs) {
    ...
    assert(`Called \`throttleTask\` on destroyed object: ${destroyable}.`, !destroyable.isDestroyed);
    const lastArgument = throttleArgs[throttleArgs.length - 1];
    const spacing = typeof lastArgument === 'boolean'
    ...
    let timers = getTimers(destroyable);
    let cancelId = throttle(destroyable, taskName, ...throttleArgs);
    timers.add(cancelId);
    return cancelId;
}
export function cancelTask(destroyable, cancelId) {
    if (typeof cancelId === 'undefined') {
        deprecate('ember-lifeline cancelTask called without an object. New syntax is cancelTask(destroyable, cancelId) and avoids a memory leak.', true, {
