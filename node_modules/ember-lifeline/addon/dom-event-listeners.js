...
export const PASSIVE_SUPPORTED = (() => {
    let ret = false;
    try {
...
/**
   ...
   */
export function addEventListener(destroyable, target, eventName, callback, options) {
    assertArguments(target, eventName, callback);
    let _callback = bind(destroyable, callback);
    ...
    if (listeners.length === 0) {
        registerDisposable(destroyable, getEventListenersDisposable(listeners));
    }
...
/**
   ...
   */
export function removeEventListener(destroyable, target, eventName, callback, options) {
    assertArguments(target, eventName, callback);
    let listeners = eventListeners.get(destroyable);
    ...
    for (let i = 0; i < listeners.length; i += LISTENER_ITEM_LENGTH) {
        if (listeners[i + ListenerItemPosition.Target] === target &&
...
function assertArguments(element, eventName, callback) {
    ...
    assert('Must provide an element (not a DOM selector)', typeof element !== 'string' &&
        typeof element !== 'function' &&
        element !== null &&
        element !== undefined);
    assert('Must provide an eventName that specifies the event type', !!eventName);
